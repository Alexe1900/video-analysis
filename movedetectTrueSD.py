...
txtOut = JA # Ausgabe in Text (JA oder NEIN)
xlOut = JA # Ausgabe in Excel (JA oder NEIN)

debugOut = JA
zeit = JA

DISKS = 5   # ANZAHL VON SCHEIBEN
source = './TEST VIDEOS/5 disks/3.mp4'    # WEG ZUM VIDEO
...
for fi in range(frameCount):
    if (fi % frameSkip == 0):
        ...
        for i in range(DISKS):
            [result] = models[i].predict(frame, stream=False, conf=confLimit)
            ...
            cv2.putText(frame, str(i), center, cv2.FONT_HERSHEY_PLAIN, 0.5, (255, 0, 0), 1)
